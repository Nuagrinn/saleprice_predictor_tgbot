...
with open('cian_data.csv', 'w', newline='', encoding='utf-8') as file:
    ...
    a_pen.writerow(('AnnouncementID', 'Title', 'Address', 'MetroStations', 'TimeToMetro', 'TotalArea',
                    'ConstructionYear', 'Price', 'Floor',
                    'Сonst/ComplYear', 'AccommodationType', 'Layout', 'Balconies', 'Renovation', 'Bathroom',
                    'WindowView', 'HouseType',
...
def file_writer(data_list):
    with open('cian_data.csv', 'a', newline='', encoding='utf-8') as file:
        ...
        for item in data_list:
            a_pen.writerow((item['AnnouncementID'], item['Title'], item['Address'], item['MetroStations'],
                            item['TimeToMetro'], item['TotalArea'], item['ConstructionYear'], item['Price'],
                            item['Floor'],
...
async def one_page_parser(ancmnt_link, headers):
    async with aiohttp.ClientSession() as session:
        async with session.get(ancmnt_link) as response:
            if response.status == 200:
                ...
                try:  # пробует найти блок с информацией "О доме"
                    ...
                    for item in inner_div:
                        try:
                            cnstr_year = item.find('div', text=re.compile('Год постройки')).find_parent('div', attrs={
                    ...
                    for item in inner_div:
                        try:
                            flr_type = item.find('div', text=re.compile('Тип перекрытий')).find_parent('div', attrs={
                                'class': 'a10a3f92e9--item--2Ig2y'}).text
                        except AttributeError:
                            pass
...
async def main(base_url, headers):
    tasks = []
    async with aiohttp.ClientSession() as session:
        async with session.get(base_url) as response:
            if response.status == 200:
                ...
                for div in divs:
                    # Генерируем псвдо-UsAgents, чтобы не отлететь в бан на асинхронном выполнении запросов
                    ...
                    tasks.append(
                        asyncio.create_task(
                            one_page_parser(ancmnt_link, headers)
                        )
                    )
...
def parser():
    ...
    # Генерируем псвдо-UserAgents, чтобы не отлететь в бан на асинхронном выполнении запросов
    user = fake_useragent.UserAgent().random
    header = {'accept': '*/*',
              'user-agent': user}
    ...
    for i in range(1, pagenation + 1):
        ...
        asyncio.run(main(base_url, headers=header))
        print(f'Времени прошло {[str(datetime.now() - dt_start)]}')
